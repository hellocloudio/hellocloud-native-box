...
kind create cluster --config ../kindconfig/kindconfig-v125.yaml
sleep 1
...
while ! $pods_checking; do
    ...
    if kubectl get pods -n metallb-system | grep -i controller | grep -q 'Running' && kubectl get pods -n metallb-system | grep -i speaker | grep -q 'Running' ; then
        ...
        kubectl create -f ../kindconfig/metallb-v0139-ipaddress-pool-3.yaml --context kind-125
...
sleep 1
kubectl config rename-context kind-125 125

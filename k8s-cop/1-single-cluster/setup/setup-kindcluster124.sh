...
kind create cluster --config ../kindconfig/kindconfig-v124.yaml
sleep 1
...
while ! $pods_checking; do
    ...
    if kubectl get pods -n metallb-system | grep -i controller | grep -q 'Running' && kubectl get pods -n metallb-system | grep -i speaker | grep -q 'Running' ; then
        ...
        kubectl create -f ../kindconfig/metallb-v0139-ipaddress-pool-2.yaml --context kind-124
...
sleep 1
kubectl config rename-context kind-124 124
